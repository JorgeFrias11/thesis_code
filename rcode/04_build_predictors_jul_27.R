...
libraries_used <- c("dplyr", "readxl", "slider", "purrr", 
                    "moments", "quantmod", "bidask", "tidyr")
...
coindata <- coindata %>%
  ...
  mutate(r2_1 = lag(ret, 1),
         dh90 = dplyr::lag(close) / slide_dbl(.x = close, 
                                              .f = max,
                                              .before = 90L,
                                              .after = -1L,
                                              .complete = TRUE)) %>%
...
coindata <- coindata %>%
  group_by(coinName) %>%
  mutate(
    skew_7d = slide_dbl(ret, skewness, .before = 7L, .complete = TRUE),
    skew_29d = slide_dbl(ret, skewness, .before = 29L, .complete = TRUE),
    kurt_7d = slide_dbl(ret, kurtosis, .before = 7L, .complete = TRUE),
    kurt_29d = slide_dbl(ret, kurtosis, .before = 29L, .complete = TRUE),
    maxret_7d = slide_dbl(ret, max, .before = 7L, .after = -1L, .complete = TRUE),
    maxret_30d = slide_dbl(ret, max, .before = 30L, .after = -1L, .complete = TRUE),
    minret_7d = slide_dbl(ret, min, .before = 7L, .after = -1L, .complete = TRUE), 
    minret_30d = slide_dbl(ret, min, .before = 30L, .after = -1L, .complete = TRUE)
    )
